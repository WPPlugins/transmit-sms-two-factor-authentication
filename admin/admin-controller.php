...
if(isset($_POST['T2FA_submitOption']) && $_POST['T2FA_submitOption'] == 'Y' && $_GET['page'] == '2FA-Transmit-SMS'){
    ...
    $T2FA_pageafter = $_POST['T2FA_pageafter'];
    $T2FA_FileType = $_POST['T2FA_FileType'];
    $T2FA_FileType = array_unique($T2FA_FileType);
    $T2FAOption = unserialize(@get_option(T2FAWpOption));
    $arrOption = array('digitCode'=>$T2FA_digitCode,'fileType'=>serialize($T2FA_FileType),'thankyouPage'=>$T2FA_pageafter);
