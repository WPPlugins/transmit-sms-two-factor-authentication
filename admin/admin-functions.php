...
function T2FASMSC_settingForm(){
    ...
    $T2FAOptionFileType = @$T2FAOption['fileType']; 
    $T2FAOptionThankyouPage = (int)@$T2FAOption['thankyouPage']; 
    if(!empty($T2FAOptionFileType)){
        $T2FAOptionFileTypeArr = unserialize($T2FAOptionFileType);
    }else {
        $T2FAOptionFileTypeArr = array();
    }
    $T2FAOptionFileTypeArrAll = array_merge($T2FA_downloadFileType,$T2FAOptionFileTypeArr);
    $offset=0;
    $limit=10;
...
<script type="text/javascript">
 ...
 var selectedInput = null;
    jQuery(document).ready(function(){
        ...
        jQuery('#T2FA_moreFileType').click(function(){
            var T2FANEWFILETYPE = jQuery('#T2FA_NewFileType').val();
            if(T2FANEWFILETYPE.length > 0){
                var ARRT2FANEWFILETYPE = T2FANEWFILETYPE.split(",");
                 for (var i = 0; i < ARRT2FANEWFILETYPE.length; i++) {
                        var newFileTypeT2FA = ARRT2FANEWFILETYPE[i].trim();
                        var T2FANEWFILETYPE_CONTENT = '<div class="col-lg-3" style="margin-bottom:5px;"><kbd><input name="T2FA_FileType[]"  checked=""  id="T2FA_FileType" type="checkbox" value="' + newFileTypeT2FA.toLowerCase() + '" class="form-control">'+ newFileTypeT2FA.toUpperCase()  +' <i class="fa fa-times" onclick="jQuery(this).parent().remove();" style="cursor:pointer"></i></kbd></div>';                        
                        jQuery('#T2FA_FileContainer').append(T2FANEWFILETYPE_CONTENT);
                    }
            }
            return false;
        })
   ...
   <div class="panel panel-info">
        ...
        <?PHP
         ...
         <table class="form-table" style="col-lg-12" cellpadding='1'>
            ...
            <tr>
                <td><label for="T2FA_apikey" style="width:150px" title="redirect to this page after download success">Page after download</label></td>
                <td>
                <?PHP wp_dropdown_pages($argsPage); ?>
              </tr>
            <tr>
                <td style="vertical-align: top;"><label for="T2FA_apikey" title="Type of downloaded file with 2FA">File Type</label></td>
                <td>
                    <div id="T2FA_FileContainer"> 
                     <?PHP
                         foreach($T2FAOptionFileTypeArrAll as $keyFile=>$valT2FAFile){
                            if(in_array($valT2FAFile,$T2FAOptionFileTypeArr)){
                                $checked = 'checked=""';
                            }else  $checked = '';
                            ?>
                            <div class="col-lg-3" style="margin-bottom:5px;">
                             <kbd><input name="T2FA_FileType[]"  <?= $checked ?>  id="T2FA_FileType" type="checkbox" value="<?=$valT2FAFile?>" class="form-control"><?=strtoupper($valT2FAFile) ?> <i class="fa fa-times" onclick="jQuery(this).parent().remove();" style="cursor:pointer"></i></kbd>
                             </div>
                            <?PHP
                         }
                    ?>
                    ...
                    <div style="margin-top: 10px;" class="col-lg-10">
                        <div class="col-lg-8">
                            <input type="text" style="width: 113%;" name="T2FA_NewFileType" id="T2FA_NewFileType" class="form-control" />
                            <em style="font-size: 11px;">Separate with commas for multiple.</em>
                        </div>
                        <div class="col-lg-1">
                            <button type="button" title="Add New File Type" name="T2FA_moreFileType" id="T2FA_moreFileType" class="btn btn-success"><i class="fa fa-plus-circle"></i>ADD</button>
                        </div>
                    </div>
                    </td>
                </tr>
            ...
            <div id="submitOptionResult" class="col-lg-12"></div>
        </div>
   </div>
