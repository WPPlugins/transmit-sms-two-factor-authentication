  ...
  public function __construct() {
   // woocommerce_thankyou_order_id
    ...
    if ( is_admin() ) {
      ...
      require_once T2FA_PLUGIN_DIR . '/admin/admin.php'; 
        ...
        if($arrOption = get_option(T2FAWpOption)){
            ...
            add_filter('the_content', array(&$this,'editContent'));
            if(isset($_POST['requestCode']) && $_POST['requestCode']=='yes'){
                ...
                exit();
            }elseif(isset($_GET['T2FAdownload']) && $_GET['T2FAdownload'] == 'yes'){
                $fileDownload = trim($_GET['filename']);
               if($this->downloadFile($fileDownload)){
    ...
    public function downloadFile($filename){
       ...
       // header('Refresh: 3;url='.$T2FAThankyouPage);
        //header('Refresh: 5;url=http://burst360.com.au/thankyoupage');
        readfile("$filename");
    ...
    public function editContent($content) {
        preg_match_all('/href="(.*?)"/i',$content,$arrLink);
        if(!empty($this->T2FAOption['fileType'])){
            $T2FAOptionFileTypeArr = unserialize($this->T2FAOption['fileType']);
        }else {
            $T2FAOptionFileTypeArr = array();
        }
        foreach($arrLink[0] as $keyLink => $valLink){
            $fileNameUrl = $arrLink[1][$keyLink];
            $file_extension = strtolower(substr(strrchr($fileNameUrl,"."),1));
            if(in_array($file_extension,$T2FAOptionFileTypeArr)){
                $replaceCode = ' onclick="return T2FAdownloadElementHandle(\''. trim($arrLink[1][$keyLink]).'\')"';
                $content = str_replace($valLink,$valLink.$replaceCode,$content);
            } 
       }  
       return $content;
    }
                    ...
                    jQuery('#T2FASMSVisitorForm').submit(function(){
                        ...
                        jQuery.ajax({
                               ...
                               success: function(result){
                                    ...
                                    if(result == 'SUCCESS'){
                                         window.location= '<?= get_site_url(); ?>?T2FAdownload=yes&filename='+fileDownload; 
                                         jQuery.ajax({
                                            url: '<?= get_site_url(); ?>?T2FAdownload=yes&filename='+fileDownload,
                                           type:'GET',
                                           success:function(){
                                               window.location= '<?=$T2FAThankyouPage?>';
                                           },
                                         });
                                    }else {
